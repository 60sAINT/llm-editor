...
export const RecognitionButton = () => {
  ...
  const { token } = useAuth();
  const { runAsync: recognition } = useRequest(async (baseUrl) => {
    const res = await sideMenuApi.recognition(baseUrl, token!);
    return res;
  });
  ...
  const handleRecognition = async (): Promise<void> => {
    ...
    if (videoBlock.type == "video") {
      ...
      const response = await recognition(removlePrefixString);
